...
# `.claude/worktrees/` (convenção conventions/git-worktree.md).
#
...
if printf '%s' "$stripped" | grep -Eq 'git([[:space:]]+-[^[:space:]]+)*[[:space:]]+worktree[[:space:]]+add'; then
  if ! printf '%s' "$cmd" | grep -q '\.claude/worktrees/'; then
    echo "BLOQUEADO: crie a worktree sob '.claude/worktrees/' (regra conventions/git-worktree.md)." >&2
    echo "Ex.: git worktree add .claude/worktrees/<escopo>-<desc> -b <tipo>/<escopo>/<desc>" >&2
    exit 2
  fi
fi
