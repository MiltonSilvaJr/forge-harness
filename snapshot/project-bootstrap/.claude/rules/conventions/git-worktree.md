...
3. **Localização canônica: `.claude/worktrees/`.** Todos os worktrees ficam **dentro do repositório**, sob `.claude/worktrees/<escopo>-<descricao>` — ex.: `.claude/worktrees/feature-foo`, `.claude/worktrees/agent-bar`. Centralizar os worktrees ali mantém a organização e alinha com o diretório que a ferramenta de worktree nativa do harness já utiliza.

4. **`.claude/worktrees/` é ignorado pelo `.gitignore`.** Worktree dentro da árvore de trabalho não pode ser versionado; a entrada no `.gitignore` é obrigatória (já contemplada pelo template do projeto).
...
git worktree add .claude/worktrees/feature-foo -b feat/payments/pix-qrcode-generation

# Criar worktree para agente de IA
git worktree add .claude/worktrees/agent-bar -b feat/finance/ledger-split-fix

# Listar worktrees ativos
git worktree list

# Remover após merge
git worktree remove .claude/worktrees/feature-foo
git branch -d feat/payments/pix-qrcode-generation
```

```bash
# Iniciar Claude Code em worktree isolado
cd .claude/worktrees/agent-bar
...
# ERRADO: criar worktree como diretório irmão do repo (fora de .claude/worktrees/)
...
git worktree add .claude/worktrees/feature-foo  # já existia de outra feature
```

## Verificação

- `git worktree list` deve mostrar apenas worktrees ativos sob `.claude/worktrees/`, com branch correspondente
- Nenhum commit diretamente em `main` por desenvolvedor ou agente
- `.claude/worktrees/` presente no `.gitignore`
