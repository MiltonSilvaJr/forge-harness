...
Antes de qualquer ação, executar o protocolo de **Bootstrap de identidade** descrito em `.claude/agents/README.md`. Este agent consome `<repo_slug>` (URLs do GitHub) e `<JIRA_KEY>` (sync Jira) — leia ambos do front-matter YAML do `AGENTS.md` raiz; faça bootstrap interativo apenas se ausentes.
...
- `/deploy-wave payment-processing dev` para iniciar promoção de ambientes
...
- Retry: re-run `/coding-status payment-processing --jira-sync`
```
...
- Próximo gate automático: `/deploy-wave payment-processing dev` após merge
```
...
```json
{
  ...
  "next_steps": [
    ...
    "/deploy-wave payment-processing dev"
  ]
}
```
...
- `.claude/agents/coding/task-coder.md` (invocador)
- `.claude/agents/coding/deploy-orchestrator.md` (próxima etapa)
- `.claude/agents/specifications/product-backlog.md` (cria issues Jira; este agente apenas consome)
