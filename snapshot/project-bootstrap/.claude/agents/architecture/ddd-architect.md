...
- O `<slug>` deriva do nome do subdomínio/BC em `kebab-case`, alinhado a `.claude/rules/conventions/naming.md`.
...
Use este esqueleto e substitua os blocos Mermaid pelo conteúdo dos arquivos `c4-level-N-*.md`. Antes de escrever o arquivo, **substitua `${project_display}` pelo valor lido do bloco YAML do `AGENTS.md`** (ver protocolo de Bootstrap em `.claude/agents/README.md#bootstrap-de-identidade`):
