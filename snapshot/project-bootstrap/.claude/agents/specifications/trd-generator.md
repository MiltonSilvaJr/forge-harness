...
> **Convenção de nomes:** todos os arquivos `.md` em **kebab-case lowercase** conforme `.claude/rules/conventions/naming.md`. Não use `TRD.md` em UPPERCASE.
...
> **Observação para projetos com domínio financeiro/auditável** (verifique `.claude/rules/domain/` e `.claude/rules/conventions/database-naming.md` se existirem neste repositório): valores monetários sempre como inteiros em centavos (`*_cents`, `BIGINT`). Tabelas em `snake_case`. Tabelas de auditoria/ledger são append-only com trigger de imutabilidade.
...
> **Observação para o projeto:** seguir `.claude/rules/architecture/security-and-secrets.md`, `.claude/rules/architecture/jwt-authentication.md`, `.claude/rules/architecture/jwt-permissions.md` e `.claude/rules/architecture/mtls-internal-services.md`. Nenhum segredo em código/imagem/repositório; mTLS interno via cert-manager + Intermediate CA.
...
> **Observação para o projeto:** seguir `.claude/rules/architecture/observability.md` (OTel + Prometheus + Loki + Jaeger; `correlationId` obrigatório; PII proibido em logs).
...
> **Observação para o projeto:** imagens de container são **multi-arch obrigatórias** (`linux/amd64` + `linux/arm64`) — ver `.claude/rules/architecture/docker-multi-arch.md` e `.claude/rules/architecture/docker-image-security.md`. Sem tag `latest` em qualquer ambiente.
...
> **Observação para o projeto:** thresholds de cobertura por camada conforme `.claude/rules/testing/quality-gates.md` (Domain ≥ 95%/90%, Application ≥ 85%/80%, Infrastructure ≥ 70%, Frontend ≥ 80%/75%). TDD obrigatório em domínio/aplicação (`.claude/rules/testing/tdd.md`).
...
> **Coerência com convenções do projeto:** seguir `.claude/rules/conventions/language-policy.md` (identificadores em inglês; documentação em pt-BR), `.claude/rules/conventions/naming.md` (kebab-case em arquivos `.md`), `.claude/rules/conventions/database-naming.md` (snake_case em PostgreSQL) e `.claude/rules/conventions/document-versioning.md` (SemVer + status do documento).
...
> **Coerência com `.claude/rules/architecture/api-and-contracts.md`:** resources em kebab-case (`/api/v1/voyage-slots`), query params em camelCase, envelope de erro com `correlationId`, breaking changes apenas em nova versão.
