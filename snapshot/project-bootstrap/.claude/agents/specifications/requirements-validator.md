...
Nunca assuma nem proponha as estruturas legadas `.kiro/specs/` ou `docs/specs/` como caminho oficial — o projeto não usa o layout do Kiro nem o intermediário `docs/specs/`. Os agentes irmãos (`requirements-writer`, `design-writer`, `tasks-writer`) e o comando `/specs-loop` operam exclusivamente em `docs/product/modules/<modulo>/`.
...
> Coerência com `.claude/rules/conventions/document-versioning.md`.
...
**Cross-ref:** Req X.Y de outro módulo, ADR-NNNN, rule `.claude/rules/...`
```
...
> Coerência com `.claude/rules/testing/tdd.md` e `.claude/rules/testing/quality-gates.md` (PBT obrigatório para `Money`, NBR 5891, splits, hashes do ledger).
...
> Coerência com `.claude/rules/conventions/language-policy.md`, `.claude/rules/conventions/naming.md` e `.claude/rules/architecture/ddd.md`.
