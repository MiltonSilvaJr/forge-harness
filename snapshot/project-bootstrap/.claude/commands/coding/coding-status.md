...
```bash
/coding-status payment-processing
```
...
Próximas ações:
   ...
   3. /deploy-wave payment-processing dev
```
...
```
/coding-status payment-processing --jira-sync
...
- **Antes de invocar `/coding-loop`**: confirmar qual módulo/onda está pronto.
...
```bash
/coding-status payment-processing --format json
```
...
- `.claude/agents/coding/task-coder.md` (gera o tracker)
- `.claude/agents/coding/sprint-orchestrator.md` (atualiza tracker quando PR abre)
- `.claude/agents/coding/deploy-orchestrator.md` (atualiza tracker pós-deploy)
- `.claude/commands/coding/coding-loop.md`
- `.claude/commands/coding/deploy-wave.md`
