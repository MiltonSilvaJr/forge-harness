...
1. Módulo tem `docs/product/modules/<modulo>/tasks.md` em status **`Aprovado para desenvolvimento`**.
   Se ainda em rascunho, rode `/specs-loop <modulo>` antes para fechar o pipeline de especificação.
...
/coding-loop payment-processing

# Executa onda específica (re-execução de onda já em progresso, por exemplo)
/coding-loop payment-processing --wave 3

# Simula sem executar
/coding-loop payment-processing --dry-run
```
...
⚠️  Operador: revise services/payment-processing/src/.../Money.cs e re-invoque /coding-loop payment-processing
```

## Referências

- `.claude/agents/coding/task-coder.md` (agent invocado)
- `.claude/agents/coding/sprint-orchestrator.md` (etapa seguinte)
- `.claude/commands/coding/deploy-wave.md` (deploy após merge)
- `.claude/commands/coding/coding-status.md` (resumo do progresso)
