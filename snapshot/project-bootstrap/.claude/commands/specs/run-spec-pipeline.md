...
- `.claude/agents/` populado com 16 agentes: discovery-agent, prd-generator, prd-validator, frd-generator, nfrd-generator, frd-nfrd-validator, ddd-architect, ddd-validator, module-generator, module-validator, trd-generator, trd-validator, requirements-writer, design-writer, tasks-writer, adr-writer
- `.claude/rules/` populado com conventions/architecture/domain/testing rules
...
3. `ls .claude/agents/specifications/ .claude/agents/architecture/` — confirmar agentes
...
6. `grep -rln -E "\.kiro/specs|docs/specs" .claude/ docs/ 2>/dev/null` — purgar referências antigas se encontrar

Se workspace tem 152MB+ de material untracked (PDFs/manuais de fornecedor), NÃO faça `git add -A`. Stage apenas `.claude/`, `docs/product/`, `discovery-notes.md`.
...
- Agente `discovery-agent`: `.claude/agents/specifications/discovery-agent.md`
- Agente `prd-generator` / `prd-validator`: idem
- Agentes `frd-*` / `nfrd-*`: idem
- Agentes `ddd-architect` / `ddd-validator`: `.claude/agents/architecture/`
- Agentes `module-*`: idem
- Agente `trd-*`: `.claude/agents/specifications/`
- Agentes `requirements-writer` / `design-writer` / `tasks-writer`: idem
- Versionamento de documentos: `.claude/rules/conventions/document-versioning.md`
- Rule de naming kebab-case: `.claude/rules/conventions/naming.md`
