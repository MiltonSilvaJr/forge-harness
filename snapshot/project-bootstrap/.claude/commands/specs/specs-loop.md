...
> **Regra inviolável:** documentos gerados por IA SEMPRE saem em status `Rascunho para revisão`. A promoção para `Aprovado para desenvolvimento` é decisão humana após ciclo multi-persona — vide `.claude/rules/conventions/document-versioning.md`.
...
- Agents disponíveis em `.claude/agents/specifications/` (requirements-writer, design-writer, tasks-writer)
...
1. Ler em paralelo:
   ...
   - `docs/adr/README.md`
   - `.claude/rules/conventions/document-versioning.md`
...
- Próxima execução de `/specs-loop` retoma desse ponto (idempotente)
