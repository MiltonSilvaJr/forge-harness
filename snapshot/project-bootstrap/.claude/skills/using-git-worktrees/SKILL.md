...
> **Fonte de verdade de naming e localização:** a política do projeto está em `.claude/rules/conventions/git-worktree.md`. Esta skill é o **procedimento acionável** que a complementa (detecção de isolamento, tool nativa, setup, baseline). Em caso de divergência, a rule vence.
...
Ferramentas nativas cuidam de posicionamento de diretório, criação de branch e limpeza automaticamente — e neste projeto já posicionam os worktrees sob `.claude/worktrees/`, exatamente a localização canônica da rule. Usar `git worktree add` quando existe uma tool nativa cria estado fantasma que o harness não enxerga nem gerencia.
...
Neste repositório, worktrees são criados **dentro do repo**, sob `.claude/worktrees/<escopo>-<descricao>` — a mesma localização que a ferramenta nativa usa. O diretório `.claude/worktrees/` é **ignorado pelo `.gitignore`** (worktree dentro da árvore de trabalho não pode ser versionado).

- **Worktree local:** `.claude/worktrees/<escopo>-<descricao>` — ex.: `.claude/worktrees/feature-pix-qrcode`, `.claude/worktrees/agent-ledger-fix`. (Sem prefixo de tecnologia, conforme a rule.)
- **Branch:** `<tipo>/<escopo>/<descricao-em-kebab-case>` — ex.: `feat/payments/pix-qrcode-generation`, `fix/finance/ledger-split-rounding`. O `<tipo>` segue os Conventional Commits do projeto (ver `.claude/rules/conventions/conventional-commits.md`).
...
git worktree add .claude/worktrees/<escopo>-<descricao> -b "<tipo>/<escopo>/<descricao>"
cd .claude/worktrees/<escopo>-<descricao>

# Exemplo concreto:
# git worktree add .claude/worktrees/feature-pix-qrcode -b feat/payments/pix-qrcode-generation
```

> **Garantia de `.gitignore`:** confirme que `.claude/worktrees/` (ou `.claude/` inteiro) está no `.gitignore` antes de criar. Se não estiver, adicione a entrada primeiro — caso contrário o worktree poluirá o `git status`.
...
| Sem tool nativa | Fallback git worktree em `.claude/worktrees/` (Passo 1b) |
| Naming / localização | Conforme `git-worktree.md` (`.claude/worktrees/<escopo>-<descricao>`) |
...
- **Problema:** criar o worktree como diretório irmão do repo (`../<escopo>-<descricao>`) quando a rule manda `.claude/worktrees/<escopo>-<descricao>`.
- **Correção:** seguir `git-worktree.md`; criar sempre sob `.claude/worktrees/`; preferência explícita do usuário vence o padrão.
...
- Criar worktree fora de `.claude/worktrees/` (ex.: diretório irmão do repo).
...
- Seguir naming e localização da rule `git-worktree.md` (`.claude/worktrees/<escopo>-<descricao>`).
...
```bash
git worktree remove .claude/worktrees/<escopo>-<descricao>
