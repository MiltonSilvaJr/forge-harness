...
- **Fixes pós-C10 (piloto azim-crm, 2026-06-10):** o teste real no Claude Code revelou (1) frontmatter YAML inválido em 3 commands (`scaffold-tdd`, `update-changelog`, `run-spec-pipeline`) — valores com `: ` ou backtick inicial sem aspas quebravam o parse e o Claude Code exibia o comando sem descrição — corrigidos com aspas; (2) `/forge:README` fantasma — o sync projetava `commands/README.md` para `.claude/commands/forge/` e o Claude Code registra qualquer `.md` ali como slash command — projeção removida. Dois novos testes no contrato (generated mode) protegem contra regressão: ausência de README projetado e frontmatter parseável com `description` em todo command.
